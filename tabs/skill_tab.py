...
class SkillTab:
    ...
    def build(self, notebook):
        ...
        headings = [("ID", "ID"), ("이름", "스킬 이름"), ("위력", "기본 위력"),
                    ("공격력배율", "공격력 배율"), ("정신력배율", "정신력 배율"), ("크리티컬확률", "크리티컬 확률")]
        ...
        ttk.Button(add_del_row, text="➕ 목록에 추가", command=self.add_skill_rule).pack(side="left", expand=True, fill="x", padx=(0, 2))
        ttk.Button(add_del_row, text="❌ 목록에서 삭제", command=self.delete_skill_rule).pack(side="left", expand=True, fill="x", padx=(2, 0))
        ...
        ttk.Button(batch_row, text="🗑️ 전체삭제", command=self.batch_clear_skills).pack(side="left", expand=True, fill="x", padx=(0, 2))
        ttk.Button(batch_row, text="↩️ 추가 옵션 초기화", command=self.batch_reset_extra_options).pack(side="left", expand=True, fill="x", padx=(2, 0))

        ttk.Label(skill_btn_frame, text="세부 옵션 편집").pack(anchor="w", pady=(20, 4))

        nav_frame = ttk.Frame(skill_btn_frame)
        nav_frame.pack(fill="x", pady=(0, 4))
        for group in SKILL_GROUPS_ORDERED:
            ttk.Button(nav_frame, text=group, width=6,
                       command=lambda g=group: self.scroll_to_group(g)).pack(side="left", padx=1)
    ...
    def _show_placeholder(self):
        for w in self.skill_detail_frame.winfo_children():
            w.destroy()
        ttk.Label(self.skill_detail_frame, text="왼쪽 목록에서 스킬을 선택하거나, ID/이름 검색으로\n스킬을 지정하면 편집 패널이 바로 열립니다.",
    ...
    def open_editor_for_id(self, sid):
        ...
        if existing is None:
            if sid not in self.app.edb_master_skills:
                if not messagebox.askyesno("경고", "순정 스킬에 없습니다. 목록에 추가하고 편집할까요?"):
                    return
    ...
    def batch_reset_extra_options(self):
        ...
        if not messagebox.askyesno(
            "추가 옵션 초기화",
            "등록된 모든 스킬의 기본 위력 / 공격력 비율 / 정신력 비율을 제외한\n나머지 세부 옵션을 기본값으로 초기화하시겠습니까?"
        ): return
        ...
        log.info(f"등록된 스킬 {len(self.cfg.current_config['skills'])}개의 추가 옵션 초기화")
        messagebox.showinfo("완료", f"등록된 스킬 {len(self.cfg.current_config['skills'])}개의 추가 옵션을 초기화했습니다.")
